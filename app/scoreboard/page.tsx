...
export default function ScoreboardPage() {
  ...
  const previousScoresRef = useRef<
    Map<string, { score: number; position: number }>
  >(new Map());
  ...
  useEffect(() => {
    ...
    const fetchScores = async (showLoading = false) => {
      try {
        ...
        const scoresWithHighlight: ScoreWithHighlight[] = newScores.map(
          (score, index) => {
        ...
        setError(
          "Impossible de charger les scores. Vérifiez votre configuration Appwrite."
        );
      } finally {
        if (showLoading) {
          setLoading(false);
  ...
  return (
    <main className="min-h-screen flex justify-center px-8 sm:px-16 py-6 sm:py-18">
      <div className="flex flex-col justify-between gap-8 w-full">
        <div className="flex flex-col lg:flex-row justify-center items-center gap-2">
          ...
          <h2 className="flex justify-center items-center gap-4 text-1xl sm:text-1xl md:text-4xl text-foreground font-mono italic">
            by
            <img src={zenika.src ?? ""} alt="Zenika" className="h-16 w-auto" />
          </h2>
        </div>
        ...
        {error && (
          <div className="bg-destructive/10 border border-destructive rounded-lg p-4 sm:p-6 text-center max-w-md">
        ...
        {!loading && !error && (
          <>
            ...
            <div className="flex flex-col lg:flex-row items-center justify-center gap-16 sm:gap-20 w-full">
              <div className="lg:max-w-[60%] lg:flex-1 bg-card rounded-lg border-2 border-primary/20 shadow-xl overflow-hidden">
                {scores.length === 0 ? (
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-primary/10">
                        {scores.map((scoreItem, index) => {
                          const hasChange =
                            scoreItem.isNew ||
                            scoreItem.scoreChanged ||
                            scoreItem.positionChanged;
                          const highlightClass =
                            scoreItem.movedUp || scoreItem.isNew
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-4 sm:px-6 py-3 sm:py-4 text-right">
                                <span
                                  className={`text-base sm:text-lg md:text-xl font-mono font-bold text-primary transition-all duration-300 ${
                                    scoreItem.scoreChanged
